...
export function ApplicationsPanel() {
  ...
  return (
    <div>
      ...
      {auth.user?.organization === "Gram Sabha" && (
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <div>
            <div className="rounded-lg border p-4 bg-secondary">
              ...
              <div className="mt-4 flex flex-col gap-3">
                ...
                <Link to="/forms/c" className="w-full text-left rounded-lg border p-4 hover:shadow-lg transition transform hover:-translate-y-1 bg-white/60">
                  <div className="flex items-center justify-between">
                    <div>
          ...
          <div className="rounded-lg border p-4 bg-white text-black">
            <h4 className="font-semibold">Raise Appeal</h4>
              ...
              <div className="mt-3">
                ...
                <div className="mt-3 flex items-center gap-3">
                  <button onClick={async () => {
                    ...
                    try {
                      await fetch('/api/appeals', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ appId: selected.id, raisedBy: auth.user?.username, raisedOrg: auth.user?.organization, targetOrg: appealTarget, type: 'appeal', message: appealMessage }) });
      ...
      {/* For other organizations: provide Raise Appeal panel to allow submitting appeals for decided applications */}
      {auth.user?.organization !== 'Gram Sabha' && (
        <div className="rounded-lg border p-4 mb-4">
          <h4 className="font-semibold">Raise Appeal (for decided applications)</h4>
          <div className="mt-3 space-y-3">
            {apps.filter((a) => a.canceled || a.currentStageIndex >= a.stages.length).map((a) => (
              <div key={a.id} className="rounded-lg border p-3 flex items-center justify-between">
                <div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { setSelected(a); setAppealMessage(''); setAppealTarget('MOTA'); }} className="rounded-md border px-3 py-1">Select</button>
                </div>
              </div>
            ))}
